...
@pytest.mark.asyncio
async def test_list_items_returns_200(client):
...
@pytest.mark.asyncio
async def test_list_items_has_seed_data(client):
...
@pytest.mark.asyncio
async def test_list_items_search(client):
    """Items search should filter by name/description."""
    resp = await client.get("/api/v1/items?q=cookie")
    items = resp.json()
    assert len(items) > 0
...
async def test_list_items_filter_category(client):
    ...
    resp = await client.get("/api/v1/items?category=kitchen")
    items = resp.json()
    for item in items:
...
@pytest.mark.asyncio
async def test_list_items_sort_oldest(client):
    """Items sorted by oldest should work."""
    resp = await client.get("/api/v1/items?sort=oldest")
    assert resp.status_code == 200
...
async def test_list_items_pagination(client):
    ...
    resp = await client.get("/api/v1/items?limit=2&offset=0")
    items = resp.json()
    assert len(items) <= 2
...
async def test_get_item_by_id(client):
    ...
    # Get an item ID from the list first
